...
- **That title is turned 3° with the band**, about its bottom left corner, so it runs parallel above it rather than cutting across. The band drops 27px over the title's width, so a level title cannot clear it evenly — sitting on the band at one end and buried at the other.
- **The two band y values above are in doubt.** 2398 puts the upper band's top 12px under that title's measured y, and both are cream, so the title lands half-invisible on it. The title was measured directly, so the bands are the ones that have moved. The title's y in code is therefore set to clear the band by 10px rather than to the measured 2386; its x is the measured 964.92. The bands' real y is still needed.
- The **"experience / tools" title has no measured position** either. Placed by eye, left-aligned to its row and sat 10px above the lower band's top edge, which is y532 at x12. Left level, that band being tilted only -1°.
